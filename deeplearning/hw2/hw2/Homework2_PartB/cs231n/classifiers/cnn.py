...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params['W2'] = np.random.normal(0, weight_scale,(num_filters*int(input_dim[1]/2)*int(input_dim[2]/2), hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss, dout = softmax_loss(l3_out, y)
        
        dout, dw3, db3 = affine_backward(dout, l3_cache)
        loss += 0.5 * self.reg * np.sum(W3**2)
        grads['W3'] = dw3 + self.reg * W3
        grads['b3'] = db3

        dout, dw2, db2 = affine_relu_backward(dout, l2_cache)
        loss += 0.5 * self.reg * np.sum(W2**2)
        grads['W2'] = dw2 + self.reg * W2
        grads['b2'] = db2

        dout, dw1, db1 = conv_relu_pool_backward(dout, l1_cache)
